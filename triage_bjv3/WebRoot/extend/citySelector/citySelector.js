 ...
 * 城市选择组件 v1.1
 * Author: yuexiang
 * Update：增加了一个参数，使得用户可以动态的设置城市列表；
 * Remark：一共有5处修改，所改之处皆有注释；
 * ---------------------------------------- *
 * Date: 2014-12-02
 * ---------------------------------------- *
 *
 * 城市选择组件 v1.1.1
 * Author: yuexiang
 * Update：解决了两个input同时调用js时，有一个参数失效的bug；
 * Remark：一共有N处修改，没有时间标注释；
 * ---------------------------------------- *
 * Date: 2014-12-03
 * ---------------------------------------- *
 * */
...
/* *
 * 【修改1】
 * Vcity.allCity为原来的城市列表（类属性？），内容是写死的，这里放弃不再使用
 * yuexiang 2014.12.2
 * */
...
/* *
 *【修改2】
 * 此处为匿名函数用法，改写了这个函数，原函数注释掉
 * 参考资料http://blog.csdn.net/yn49782026/article/details/8549462
 * yuexiang 2014.12.2
 * */
/*
(function () {
...
/* *
 *【修改3】
 * 改写了原匿名函数，新函数命名为adapterCity，增加了一个形参citylist取代原来的Vcity.allCity
 * yuexiang 2014.12.2
 */
function adapterCity (citylist) {
	Vcity.oCity = {hot:{}};
	if(!Vcity.oCity.hot['hot']) Vcity.oCity.hot['hot'] = [];
	for (var i = 0, n = citylist.length; i < n; i++) {
		Vcity.oCity.hot['hot'].push(citylist[i]);
	}         
};
...
Vcity.CitySelector.prototype = {
    ...
    /* 初始化 */
		...
		/* *
		 *【修改4】
		 * 初始化函数增加一个参数istr，并在初始化时调用adapterCity函数
		 * yuexiang 2014.12.2
		 */
    initialize :function (options,istr) {
        ...
        this.inputEvent(istr);
        
        /*读取属性
        var ob=eval(Vcity.oCity.ABCDEFGH.D);
　　    var property="";
　　    for(var p in ob){
　　         property+=p+"\n";
　　    }
　　    alert(property);
        alert(Vcity.oCity.ABCDEFGH.D[0]);*/
    },
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                $(that.input).val(this.innerHTML).change();
                //其他项目去掉以下js代码
                
                var inputId = $(that.input).attr("id");
                
                if((inputId=='sg')||(inputId=='tz')||(inputId=='tiwen')||(inputId=='maibo')
                		||(inputId=='huxi')||(inputId=='shuzhangya')
                		||(inputId=='shousuoya')||(inputId=='spo2')){
                	//alert(inputId);
    ...
    inputEvent:function(iestr){
        var that = this;
        Vcity._m.on(this.input,'click',function(event){
            event = event || window.event;
            if(!that.cityBox){
                that.createWarp(iestr);
    ...
    createUl:function () {
        ...
        if (value !== '') {
            ...
            // 如果slideul不存在则添加ul
            if (!this.ul && searchResult.length != 0) {
