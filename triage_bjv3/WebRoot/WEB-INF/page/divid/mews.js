...
function getMewsScore(){
	...
	if(!$("#mews_score_div").length>0){
		...
		str+= "急诊预检分级分诊指标总积分 = <span id=\"mews_score\">0</span>";
	    str+= "</div>";
	    html = html +str;
	    $("#symptom").html(html);
	    $("#symptomhide").val(html);
	}
    ...
    if(old==""||old>grade||!document.getElementById("edts_x")){
    	$("#autograde").val(grade);
    	...
    	if($("#finalgrade").val()==""){
    		...
    		$("#grade").html(html);
    		$("#zdpj").html("");
	    	var html = '<p class="text-center">综合评级:</p><div class="st-grade  btn-'+grade+'-selected">';
    		html+= '<div><p class="allocatNum">'+name+'</p>';
    		html+= '<p class="allocatUnit">级</p>';
    		html+= '</div></div>';
	    	$("#zdpj").html(html);
    		$("#twopartpj").hide().siblings(".title-top").show();
