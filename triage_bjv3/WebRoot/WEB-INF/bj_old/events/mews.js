function initMews() {
	$("#mews_a li").bind(
			...
			function() {
				...
				if ($(this).is('.active')) {
					...
					$(".mews_score_A").text(
							parseInt(mews_score_A) - parseInt(5));
				} else {
					...
					function() {
						...
						if ($(this).is('.active')) {
							...
							$(this).addClass('active').siblings().removeClass(
									'active');
							var mews_score_B = $(".mews_score_B").text();
							mews_score_B_New = parseInt(mews_score_B)
									+ parseInt(sel_score);
	...
	$("#mews_c li").bind("click",function(){ 
		...
		var mews_sign_score =  td.text();
		if($(this).is('.active')){
			$(this).removeClass('active');
			td.html(parseInt(mews_sign_score)-parseInt(sel_score));
		}else{
			var old_score = $(this).siblings('.active').text();
			if((old_score!=null)||(old_score!="")){
			...
			mews_sign_score_New= parseInt(mews_sign_score)+parseInt(sel_score);
			td.html(mews_sign_score_New);
		}
		getMewsScore();
	});
}
...
function getMewsScore() {
	...
	if (!$("#symptom").find("#mews_score_div").length > 0) {
...
//	var score_huxi = getMewsHuxiScore(huxi);
	...
	var score_tiwen = $('#mews_tiwen_score').text();
	score_tiwen = parseInt(score_tiwen);
	
	var score_yishi = $('#mews_yishi_score').text();
	score_yishi = parseInt(score_yishi);

	var score_xueya = $('#mews_xueya_score').text();
	score_xueya = parseInt(score_xueya);
	
	var score_maibo = $('#mews_maibo_score').text();
	score_maibo = parseInt(score_maibo);
	
	var score_huxi = $('#mews_huxi_score').text();
	...
	$("#mews_score").text(
			parseInt(mews_score_A) + parseInt(mews_score_B)
					+ parseInt(mews_score_C));
	
	$("#symptom").find("#mews_score").text($("#mews_score").text());
	...
	tr.find("input[name='mewsgrade']").val("100"+grade);
	var at_grade = getHigestGrade();
	showAutoGrade(at_grade);
	if($("#mews_score").text()=='0'){
		$("#mews_x").click();
	}
}
