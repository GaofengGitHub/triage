...
//	$("#changereason option:first").prop("selected", 'selected');
	...
	function save(){
	    ...
	    for (var i=0;i<trList.length;i++) {
	    	...
	    	if(fullname==""){
//	    		alert(tr.find("input[name='fullname']").attr('value'));
	    		fullname = tr.find("input[name='fullname']").attr('value');	
//	    		alert(fullname);
...
function removerow() {
	...
	var tr = this.getRowObj(row);
	if (tr != null) {
		tr.parentNode.removeChild(tr);
	}
	var num = $("#tbody tr").length; // 获取tr的长度
	for ( var i = 0; i <= num; i++) { // 进行循环
		$("#tbody tr .td_center").eq(i).html(i + 1);
	}
}
