...
//	
	...
	*/
		...
		$(".checkDiv>div .checked-static").click(function(){
			...
			$(this).parents(".col-md-2").siblings().find(".triage-other-box").removeClass("active");
			$(this).parents(".col-md-2").siblings().find(".triage-other-box").find("input.other-control").hide();
		});
		$(".checkDiv>div .checked-static").click(function(){
			$(this).parents(".triage-other-box").addClass("active").parent().siblings().removeClass("active");
			$(this).parents(".triage-other-box").find("input.other-control").show().css("opacity","1");
...
//			alert($(this).hasClass('active'));
			if(!$(this).hasClass('active')){
				$(this).children("span").children("input").prop("checked",false);
			}else{
...
function readCard(type){
	...
	if(cardnum !="" || $("#idcard").val()!=""){
		cardnum = cardnum.replace(';','').replace('?','').replace(':','').replace('？','').replace(':','').replace('；','');
		$("#cardnum").val(cardnum);
		$.ajax({
	        type: "post",
	        url: "#path()/divid/queryPatientByCardNum",
	       		     ...
	       		     //if($("#fullname").val()=="" && $("#bornday").val()=="" && $("#gender").val()=="" && $("#tel").val()=="" && $("#idcard").val()==""){
	          		    //填入信息
	                   	...
	                   	if((date.getTime() - dividtime2.getTime())/(24*3600*1000)<=1){
	                   		...
	                   		if(project_name =='bb'||project_name =='zy'||($("#fullname").val()=="" && $("#bornday").val()==""&& $("#tel").val()=="" && $("#idcard").val()=="")){
	                   			var birthday = info.bornday.substring(0,10);
	                       		$("#fullname").val(info.fullname);
	                       		$("#bornday").val(birthday);
	                       		$("#gender").val(info.gender);
...
//	                     	    });
	                       		if (typeof(info.anamnesis) != "undefined"){
...
//				        					$("input[name='anamnesis'][value='"+info.anamnesis+"']").parent().parent().attr("class","col-md-2 col-li active");
	                       		...
	                       		if (typeof(info.allergic_history) != "undefined"){
    		...
    		if(result.status==200){
    			if(result.rows.length>0){ 
    				...
    				if( $("#returnUrl").val()=="xgpj" || $("#returnUrl").val()=="histroy"|| savestatus=="save" && $("#returnUrl").val()=="divid" ){
    					...
    					if(versionCheck("index_receive_patients_table_divid")){
...
//	      				strhtml += '<label class="control-label">'+result.rows[i].name+'</label><span class="checked-static radio-static">';
//	      				strhtml += '<input type="radio" name="source" value='+result.rows[i].name+'>';
//	      				strhtml += '<i></i></span></div>';
	...
	$.ajax({
	      ...
	      success: function(result){
	      	if((result!=null)&&(result.status==200)){
	      		var strhtml = ""
	      		for(var i=0;i<result.rows.length;i++){
	      			strhtml += '<div class="col-md-2"><label class="control-label">'+result.rows[i].name+'</label><span class="checked-static radio-static">';
...
function cancel(){
	...
	if(finalgrade ==''){
		...
		$("#finalgradearea").html("");
		var str = '';
		str += '<div class="st-grade btn-'+finalgrade+'-selected" style="height:64px;">';
		str += '<p class="allocatNum"  style="line-height: 64px;">'+finalgrade.substring(3,4)+'</p>';
		str += '<p class="allocatUnit">级</p>';
		str += '</div></div>';
		$("#finalgradearea").html(str);
	}
