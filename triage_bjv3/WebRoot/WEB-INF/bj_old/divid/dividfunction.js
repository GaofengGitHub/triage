$(document).ready(function(){	
	...
	//添加验证
	$("#tiwen,#spo2,#shousuoya,#maibo").bind("input propertychange",function(event){
		showSignGrade(false);
	});
...
function tooltip(_this,content,direction){
	clearInterval(time);
	num++;
	var tool = '<div class="pt" id="'+num+'">'
		+'<div class="cont"><i class="glyphicon glyphicon-exclamation-sign icongly"></i>'
		+content+'</div><b class="out"></b><b class="in"></b></div>';
	$("body").append(tool);	
...
//			}
//		}
		re =/^((13[0-9]{1})|(14[5,7]{1})|(15[0-3,5-9]{1})|(17[0,1,3,5-8]{1})|(18[0-9]{1})|(166)|(198)|(199)|(147))+\d{8}$/;
		/*if(tel.length>0){
			if(!re.test(tel)){ 
		...
		for(var i=0;i<collist.length;i++){
			...
			if(col=='cardnum'){
				...
				if(!cardnum.length>0 &&necessary=='1'){
					if(fullname.indexOf('无名氏')== -1&& !state){
						$("#cardnum").parents(".rank").addClass("remind").siblings().addClass("assist");
						$("#cardnum").parents(".form-group").addClass("padd-t");	
						var cont = '【就诊卡号】不能为空，</br>请读取或输入就诊卡号。';
						var dom = $('#cardnum');
...
//					console.log($("#bornday").parents(".form-group").attr('class','padd-t'));
					$("#bornday").parents(".form-group").attr('class','form-group clearfix padd-t')
...
function showRct(grade,handle){
	...
	if(grade =='1003' || grade=='1004' || finalgrade =='1003' || finalgrade=='1004'){
		...
		if(versionCheck("divid_patient_intoresuce")){
			$("#div_go_rct").show();
		}
